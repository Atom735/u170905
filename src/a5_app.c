...
void A5APP_Idle(pA5S_App pApp) {
    ...
    if(pApp->FT_Cache->uFlagUpdate) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, pApp->FT_Cache->uCacheSize, pApp->FT_Cache->uCacheSize, 0, GL_RED, GL_UNSIGNED_BYTE, pApp->FT_Cache);
    ...
    if(pApp->iDbgState == 1) {
        ...
        // }
        A5GL_2DTXT_DrawSetTex(&pApp->gl.s2DTXT, pApp->gl.tex_FT_Cache, pApp->FT_Cache->uCacheSize);
        ...
        A5S_TXT_Settings ts;
        GLuint uCount = 0;
        SDL_memset(&ts, 0, sizeof(A5S_TXT_Settings));
        if(SDL_HasClipboardText()) {
            ts.pzTxtChar = SDL_GetClipboardText();
            uCount = pApp->gl.vsz_DBG_CLIPBOARD;
        } else {
            ts.pzTxtChar = SDL_malloc(32);
            uCount = 11;
            SDL_memcpy(ts.pzTxtChar, "No Clipboard", 13);
        }

        ts.pCache = pApp->FT_Cache;
        ts.uFontID = 1;
        ts.uHeight = pApp->gl.hsz_DBG_CLIPBOARD;
        ts.r = 0xef;
        ts.g = 0xff;
        ts.b = 0xdf;
        ts.a = 0xff;
        ...
        SDL_free(ts.pzTxtChar);
        A5GL_2DTXT_DrawVbo(&pApp->gl.s2DTXT, 8-ms_x*8, 8-ms_y*4+ts.uHeight, pApp->gl.vbo_DBG_CLIPBOARD, 0, strSz);
    }
