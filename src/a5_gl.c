...
GLvoid A5GL_2DTXT_Release(pA5GLV_2DTXT p) {
    glDeleteProgram(p->p);
    glDeleteShader (p->v);
    glDeleteShader (p->f);
    SDL_memset(p, 0, sizeof(A5GLV_2DTXT));
}
GLuint A5GL_2DTXT_CreateVBO_D(GLuint uSize) {
    GLuint o;
    glGenBuffers(1, &o);
    glBindBuffer(GL_ARRAY_BUFFER, o);
    glBufferData(GL_ARRAY_BUFFER, sizeof(A5V_2DTXT)*uSize*6, NULL, GL_DYNAMIC_DRAW);
    return o;
}
GLuint A5GL_2DTXT_CreateVBO_S(GLuint uSize, pA5V_2DTXT pBuf) {
    GLuint o;
    glGenBuffers(1, &o);
    glBindBuffer(GL_ARRAY_BUFFER, o);
    glBufferData(GL_ARRAY_BUFFER, sizeof(A5V_2DTXT)*uSize*6, pBuf, GL_STATIC_DRAW);
    return o;
}
...
GLvoid A5GL_2DTXT_DrawVbo(pA5GLV_2DTXT p, GLint iPosX, GLint iPosY, GLuint uVBO, GLuint uOffset, GLuint uCount) {
    ...
    glVertexAttribPointer(p->_aPos, 2, GL_SHORT, GL_FALSE, sizeof(A5V_2DTXT), &((pA5V_2DTXT)0)->x);
    glVertexAttribPointer(p->_aTex, 2, GL_SHORT, GL_FALSE, sizeof(A5V_2DTXT), &((pA5V_2DTXT)0)->u);
    glVertexAttribPointer(p->_aCol, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(A5V_2DTXT), &((pA5V_2DTXT)0)->r);
...
GLuint A5GL_CreateShader(const char *szFileName, GLenum eType) {
    const GLchar *source = (const GLchar *)A5UT_FileRead(szFileName, NULL);
    if (!source)
        return SDL_LogError(SDL_LOG_CATEGORY_RENDER, "Error opening \"%s\"", szFileName), 0;
    GLuint res = glCreateShader(eType);
    glShaderSource(res, 1, &source, NULL);
    SDL_free((void*)source); source = NULL;
...
GLuint A5GL_CreateProgram2(GLuint uiVS, GLuint uiFS) {
...
#define OPENGL_GET_PROC(p,n) \
    if ((n = (p)SDL_GL_GetProcAddress(#n))) ++i; else \
        SDL_LogError(SDL_LOG_CATEGORY_RENDER, "Loading extension '%s' fail\n", #n);
#define OPENGL_STOP_GET_PROC(c, v) \
    iM += c; if(i>=iM) o=v; \
    SDL_LogInfo(SDL_LOG_CATEGORY_RENDER, "Loaded %d/%d extension of OpenGL %d.%d\n", \
        i, iM, v>>4, v&0xf);
